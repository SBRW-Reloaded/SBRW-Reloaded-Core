...
public class LeaderboardDAO {
    ...
    public LeaderboardEntity getResultByNameAndEventId(int eventId, String personaName, Boolean isPowerup) {
        ...
        Query query = entityManager.createNativeQuery("SELECT RANKING, TIMES, PERSONANAME FROM (SELECT ROW_NUMBER() OVER (ORDER BY EVENT_DATA.eventDurationInMilliseconds ASC) AS RANKING, PERSONA.name AS PERSONANAME, COUNT(BAN.active) AS BANNED, EVENT_DATA.eventDurationInMilliseconds AS TIMES FROM EVENT_DATA INNER JOIN PERSONA ON PERSONA.ID = EVENT_DATA.personaId LEFT JOIN BAN ON BAN.user_id = PERSONA.USERID WHERE EVENT_DATA.EVENTID = :eventid AND EVENT_DATA.finishReason = 22 AND EVENT_DATA.hacksDetected IN(0,22,32) "+appendPuCommand+" GROUP BY PERSONA.name HAVING 1 AND BANNED = 0) x WHERE PERSONANAME = :personaname", LeaderboardEntity.class);

        query.setParameter("eventid", eventId);
        query.setParameter("personaname", personaName);
    ...
    public LeaderboardEntity getBestPlayerByEventId(int eventId, Boolean isPowerup) {
        ...
        Query query = entityManager.createNativeQuery("SELECT RANKING, TIMES, PERSONANAME FROM (SELECT ROW_NUMBER() OVER (ORDER BY EVENT_DATA.eventDurationInMilliseconds ASC) AS RANKING, PERSONA.name AS PERSONANAME, COUNT(BAN.active) AS BANNED, EVENT_DATA.eventDurationInMilliseconds AS TIMES FROM EVENT_DATA INNER JOIN PERSONA ON PERSONA.ID = EVENT_DATA.personaId LEFT JOIN BAN ON BAN.user_id = PERSONA.USERID WHERE EVENT_DATA.EVENTID = :eventid AND EVENT_DATA.finishReason = 22 AND EVENT_DATA.hacksDetected IN(0,22,32) "+appendPuCommand+" GROUP BY PERSONA.name HAVING 1 AND BANNED = 0) x WHERE RANKING = 1", LeaderboardEntity.class);
